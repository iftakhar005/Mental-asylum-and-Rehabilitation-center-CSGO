...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['appointment_id'], $_POST['status'])) {
    ...
    if (in_array($status, ['scheduled', 'completed', 'cancelled'])) {
        $stmt = $conn->prepare("UPDATE appointments SET status = ? WHERE id = ? AND therapist = ?");
        $stmt->bind_param("sis", $status, $apt_id, $therapist_staff_id);
        if ($stmt->execute()) {
...
$stmt = $conn->prepare("SELECT a.*, p.full_name as patient_name FROM appointments a JOIN patients p ON a.patient_id = p.patient_id WHERE a.therapist = ? ORDER BY a.date DESC, a.time DESC");
$stmt->bind_param("s", $therapist_staff_id);
$stmt->execute();
...
<head>
    <meta charset="UTF-8">
    <title>All Appointments - Therapist</title>
...
<body>
    <div class="container">
        <a href="therapist_dashboard.php" class="back-link"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>
