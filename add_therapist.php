...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $staff_id = 'THR-' . date('Ymd') . '-' . rand(1000, 9999);
    // $email = generateEmail($firstName, $lastName, 'therapist'); // Removed auto-generation
    ...
    try {
        ...
        $sql = "INSERT INTO staff (staff_id, full_name, role, dob, gender, address, experience, phone, password_hash, status, email, user_id, temp_password, two_factor_enabled) VALUES ('$staff_id', '$full_name', 'therapist', '$dob', '$gender', '$address', '$experience', '$phone', '$hashed_password', 'Active', '$email', $user_id, '$temp_password', $enable_2fa)";
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            ...
            <p class="page-subtitle">Fill in the details below to add a new therapist to the system</p>
        </div>
        <?php if ($success_data): ?>
        ...
        <?php endif; ?>
        <div class="card">
            <form id="staffForm" method="POST" style="display: flex; flex-direction: column; gap: 1.2rem;">
                <div style="display: flex; align-items: center; gap: 0.7rem; margin-bottom: 0.5rem;">
                    <i class="fas fa-user-plus" style="font-size: 1.5rem; color: var(--primary);"></i>
                    <h2 style="margin: 0; font-size: 1.4rem; font-weight: 700;">Add New Therapist</h2>
                </div>
                ...
                <div style="display: flex; gap: 1rem; margin-top: 1.2rem;">
                    <button type="button" class="btn" style="background: linear-gradient(135deg, var(--secondary), var(--primary));" onclick="window.location.href='dashboard.php'">Cancel</button>
                    <button type="submit" class="btn" style="background: linear-gradient(135deg, var(--primary), var(--secondary));">Save Staff</button>
                </div>
            </form>
        </div>
    </div>
